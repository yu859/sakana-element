...
const transitionEvents: Record<string, (el: HTMLElement) => void> = {
  //Record表示一个对象类型，key是string类型，value是(el: HTMLElement) => void类型
  beforeEnter(el) { _setHeightZero(el), _setOverflowHidden(el)},
  enter: (el) => _setHeightScroll(el),
  afterEnter(el) {_setHeightEmpty(el),_setOverflowEmpty(el)},
  beforeLeave(el) {_setHeightScroll(el),_setOverflowHidden(el)},
  leave: (el) => _setHeightZero(el),
  afterLeave(el) {_setHeightEmpty(el),_setOverflowEmpty(el)}
}
